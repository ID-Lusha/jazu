...
type AuthStatus = { ok: boolean } | null;
...
/** Сбросить кэш — вызывать после login/logout, чтобы навигация
 * перерисовалась без F5. */
export function resetAuthStatus() {
  cached = null;
  inflight = null;
  // Мгновенно дёрнем новую загрузку, чтобы подписчики получили апдейт.
  void ensureLoaded();
}
