...
export async function buildRuntimeTurn(
  ...
  const runtimeSystem = [
    ...
    "Ты отвечаешь клиенту в WhatsApp как живой менеджер компании. Не упоминай, что ты AI, если это явно не нужно.",
    "Если запрос требует человека — подготовь handoff summary и предупреди клиента, что передаёшь специалисту.",
    "",
    "ВАЖНО: верни СТРОГО валидный JSON без markdown-обёртки, формат:",
    "{",
    "  \"reply\": string — твой ответ клиенту (так, как его получит человек в WhatsApp),",
